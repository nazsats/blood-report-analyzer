        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div
            className={`flex justify-between items-center px-4 sm:px-6 h-14 rounded-2xl transition-all duration-500 ${scrolled
                ? "bg-gray-950/90 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-2xl shadow-black/40"
                : "bg-transparent"
              }`}
          >
            {/* Logo */}
            ...
            <div className="hidden md:flex items-center gap-1">
              {user && navLinks.map((link) => (
                <Link
                  key={link.name}
                  href={link.href}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-400 hover:text-white hover:bg-white/8 transition-all duration-200"
                >
            ...
            <div className="hidden md:flex items-center gap-2">
              ...
              <button
                ...
                className="p-2 rounded-xl text-gray-600 dark:text-gray-400 hover:text-white hover:bg-white/8 transition-all duration-200"
              >
                {mounted ? (
              ...
              {user ? (
                ...
                <>
                  <button
                    ...
                    className="px-4 py-2 rounded-xl text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-white hover:bg-white/8 transition-all duration-200"
                  >
                    Sign In
                  </button>
                  <motion.button
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden mx-4 mt-2 rounded-2xl bg-gray-950/95 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-2xl overflow-hidden"
            >
              <div className="p-4 space-y-1">
                {user && navLinks.map((link, i) => (
                  ...
                  >
                    <Link
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/8 text-gray-700 dark:text-gray-300 hover:text-white font-medium transition-all"
                    >
                ...
                {!user && (
                  <motion.button
                    ...
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/8 text-gray-700 dark:text-gray-300 hover:text-white font-medium transition-all"
                  >
                    <div className="w-8 h-8 rounded-lg bg-primary-500/15 flex items-center justify-center">
